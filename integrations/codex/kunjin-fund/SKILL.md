...
当用户问“最近哪些行业有变化”或点名一个领域时，先由外层按需查询本地已核验的公开指标覆盖，
再做有限、多渠道的公开网页发现与逐页读取：搜索引擎只用于定位，优先政府、监管、交易所、基金公司
和行业协会原文，其次是有明确数据归属的大型平台；媒体、社区只作待核对线索。每个方向只做有限的
主入口与一次可信替代入口尝试，失败后立即显示来源缺口，不做长时间轮询、后台抓取或任意 URL 代理。
外层验证来源、发布日期、统计期、数值/单位和口径后，用 `research evidence-store` 写入结构化事实；
