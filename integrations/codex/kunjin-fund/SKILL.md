...
1. 先运行 `research local-overview`，再运行 `research scan`。前者无需用户指定领域或指标，列出持久化指标、
   事件三态、每条指标的已覆盖期、待补期与修订检查期；后者合并旧 intelligence 与这些本地事实，最多提出三个候选。
2. 对每个候选方向做一次搜索引擎或市场热点页发现，搜索结果只用于定位，不作为最终事实；宽泛问题同时找
   连续行业指标和当日/近期市场热点，最多保留 3 至 5 个方向。
3. 直接读取一个优先来源：政府、监管、交易所、基金公司/上市公司正式公告或行业协会原文；对“板块上涨、
   涨停”优先读取可核验行情事实页。
4. 主页不可读或字段不足时，只尝试一次可信替代：大型结构化财经平台或可信财经媒体原文。
   百度百家号、论坛、社区和自媒体仅保留为发现线索；验证码、访问限制或两次入口失败后立即降级并显示缺口，
   不轮询、不绕过限制、不把 CLI 变成通用爬虫。
5. 按事件键、原始 URL、发布主体、统计期和数值去重；事件键由外层按领域、事件日期、主体/板块和事件类型
...
`local-overview` / `scan` 返回的 `outer_discovery_required=true` 是完成门，不是联网成功标记。最终中文回答前，
外层必须记录一次受控发现为 `completed`，或在没有浏览器能力、入口受限或字段不足时明确标记 `blocked`；
未完成时只能说“本次未完成互联网近期刷新”，不能声称已主动查询。随后才读取一次本地组合并做带日期披露的基金映射。
