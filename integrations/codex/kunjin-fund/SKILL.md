...
当用户宽泛询问“最近市场/哪些板块”而当日结构化行情入口失败、超时或只覆盖局部板块时，不得直接结束为
“仅有局部行情”。从已发现方向中至多保留三个，逐方向执行发现计划：一次搜索发现 -> 读取一个近一周可信直接页
...
当一份自然中文回答同时使用 `portfolio show`、`portfolio diagnose`、`fund candidates` 或带
`--portfolio-context cached` 的 `fund review` 时，必须比较各结果的 `portfolio_snapshot`：
`sync_run_id`、`observation_version`、`observation_at` 与 `positive_position_fingerprint` 必须一致。
任一后续步骤触发或观察到新快照时，旧的 HHI、组合权重、候选映射和单基金组合上下文都视为
过期；最多重新读取一次 `portfolio show` / `portfolio diagnose` 并重跑依赖组合的候选或复核。
若只需把已取得的基金/市场事实重新绑定到新组合，内部使用 `fund review --cached-public-context`
跳过 brief 与近期市场刷新；结果必须标注 `public_context_refresh=not_refreshed`，不得把它说成当日市场已刷新。
若重算期间再次变化或命令返回 `stale_unmerged`，只能说明本次组合结论不可合并，不能把旧诊断
与新基金权重放在同一结论中。零份额历史条目不计入 `positive_position_count`；未识别主题持仓也不等于
底层零暴露。
...
普通“是否继续持有”的条件研究不需要交易授权：应急资金、期限、风险承受或近期用途缺失时，继续提供已核验
基金事实与条件性观察，并明确哪些个人边界尚待补充；只有已知应急资金不足或近期必须用钱时，才把风险边界升级为
“先降低风险”。不得把未更新画像表述成“无法形成继续持有授权”。
