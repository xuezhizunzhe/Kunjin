...
class LaunchdTest(unittest.TestCase):
    def test_plist_runs_weekdays_after_close(self) -> None:
        ...
        self.assertEqual([item["Weekday"] for item in plist["StartCalendarInterval"]], [2, 3, 4, 5, 6])
        self.assertTrue(all(item["Hour"] == 18 and item["Minute"] == 30 for item in plist["StartCalendarInterval"]))
