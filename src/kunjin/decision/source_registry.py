...
def _build_sources() -> Tuple[SourcePolicy, ...]:
    return (
        SourcePolicy(
...
class SourceRegistryV1:
    ...
    def validate(self) -> None:
        ...
        expected_sources = _build_sources()
        if type(self.sources) is not tuple or self.sources != expected_sources:
        ...
        for source in self.sources:
            ...
            for source_field in source.fields:
                identities.add(SourceFieldRef(source.source_id, source_field.field_id))
                alternatives.extend(source_field.acceptable_alternatives)
        ...
        identity_pairs = frozenset(
            (reference.source_id, reference.field_id) for reference in identities
        )
        if identity_pairs != V1_SOURCE_FIELD_IDENTITIES:
            raise ValueError("source registry identities differ from the runtime V1 contract")
        expected_tiers = dict(V1_SOURCE_TIERS)
        for source in self.sources:
            if any(
                source_field.source_tier is not expected_tiers[source.source_id]
                for source_field in source.fields
            ):
                raise ValueError("source registry tier differs from the runtime V1 contract")
